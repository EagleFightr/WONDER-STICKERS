/* javascript.js - Wonder Stickers */

/* ---------- Config ---------- */
var STORAGE_KEY = 'ws_cart_v1';
var CURRENCY = { locale: 'en-IN', code: 'INR' };
var WHATSAPP_NUMBER = '919220727618'; // your WhatsApp number (country code, no plus)
...
function loadCart() {
  ...
  } catch (e) { cart = {}; console.error('loadCart error', e); }
  window.cart = cart;
}

/* ---------- Helpers ---------- */
function fmtCurrency(n) {
  try { return new Intl.NumberFormat(CURRENCY.locale, { style: 'currency', currency: CURRENCY.code }).format(n); }
...
function renderProductsGrid() {
  ...
  PRODUCTS.forEach(function (p) {
    ...
    card.className = 'card';
    var imgHtml = p.img ? '<div class="thumb"><img src="' + p.img + '" alt="' + (p.name || '') + '" class="product-img"></div>'
                        : '<div class="thumb"><span>' + (p.name ? p.name[0] : '') + '</span></div>';
...
function removeFromCart(id) {
  if (cart[id]) { delete cart[id]; saveCart(); updateUI(); }
}
function computeTotal() {
  return Object.keys(cart).reduce((t, id) => {
    var p = getProductById(id); var qty = cart[id] || 0;
    return t + ((p ? p.price : 0) * qty);
  }, 0);
}

/* ---------- Update UI ---------- */
function updateUI() {
  var qty = Object.keys(cart).reduce((s, k) => s + Number(cart[k] || 0), 0);
  var totalValue = computeTotal();
  var fabCount = document.getElementById('fabCount'); if (fabCount) fabCount.textContent = qty;
  var cartTotalShort = document.getElementById('cartTotalShort'); if (cartTotalShort) cartTotalShort.textContent = fmtCurrency(totalValue);
  var cartTotal = document.getElementById('cartTotal'); if (cartTotal) cartTotal.textContent = fmtCurrency(totalValue);
  var drawerTotal = document.getElementById('drawerTotal'); if (drawerTotal) drawerTotal.textContent = fmtCurrency(totalValue);

  var itemsHtml = qty === 0 ? '<div class="muted small">Your cart is empty</div>' : '';
  if (qty > 0) {
    Object.keys(cart).forEach(id => {
      var p = getProductById(id); var q = cart[id] || 0; if (!p) return;
      ...
      itemsHtml += '<div class="cart-item">'
        ...
        + '<div class="small muted">' + fmtCurrency(p.price) + ' × ' + q + ' = <strong style="color:var(--accent)">' + fmtCurrency((p.price || 0) * q) + '</strong></div></div>'
        + '<div><button class="remove" data-id="' + id + '">Remove</button></div>'
        + '</div>';
    });
  }
  var cartItems = document.getElementById('cartItems'); if (cartItems) cartItems.innerHTML = itemsHtml;
  var mobileItems = document.getElementById('mobileCartItems');
  if (!mobileItems) {
    var drawer = document.getElementById('cartDrawer');
    if (drawer) { mobileItems = document.createElement('div'); mobileItems.id = 'mobileCartItems'; drawer.insertBefore(mobileItems, drawer.firstChild); }
  }
  if (mobileItems) mobileItems.innerHTML = itemsHtml;

  var remButtons = document.querySelectorAll('.remove');
  remButtons.forEach(b => {
    if (!b._wsRemoveAttached) { b.addEventListener('click', () => removeFromCart(b.dataset.id)); b._wsRemoveAttached = true; }
  });
...
function attachDrawerHandlers() {
  ...
  if (fab && !fab._wsAttached) { fab.addEventListener('click', () => { if (drawer) drawer.classList.add('open'); if (backdrop) backdrop.style.display = 'block'; }); fab._wsAttached = true; }
  if (backdrop && !backdrop._wsAttached) { backdrop.addEventListener('click', () => { if (drawer) drawer.classList.remove('open'); backdrop.style.display = 'none'; }); backdrop._wsAttached = true; }
}

/* ---------- WhatsApp checkout ---------- */
function buildWhatsAppMessage(phone) {
  var cartObj = (window.cart && Object.keys(window.cart).length) ? window.cart : {};
  if (!Object.keys(cartObj).length) { try { var raw = localStorage.getItem(STORAGE_KEY); cartObj = raw ? (JSON.parse(raw) || {}) : {}; } catch (e) { cartObj = {}; } }
  if (!Object.keys(cartObj).length) return null;

  var lines = ['Hey Ranvir 👋 I want to order these items from Wonder Stickers:', ''];
  var total = 0;
  Object.keys(cartObj).forEach(id => {
    var qty = Number(cartObj[id] || 0); if (!qty) return;
    var p = getProductById(id) || { name: id, price: 0 };
    var sub = (p.price || 0) * qty; total += sub;
    lines.push('- ' + p.name + ' × ' + qty + ' = ' + fmtCurrency(sub));
  });
  lines.push('', '----------------------', 'Total = ' + fmtCurrency(total), '', 'Customer phone: ' + (phone || '(please reply with your phone)'), '', 'Name:', 'Address:', '', 'Please confirm availability & payment instructions. Thanks!');
  return lines.join('\n');
}
function openWhatsAppCheckout(phone) {
  ...
  if (isMobileDevice()) {
    ...
    try {
      ...
      setTimeout(() => window.open('https://wa.me/' + WHATSAPP_NUMBER + '?text=' + encoded, '_blank'), 600);
    } catch (e) { window.open('https://wa.me/' + WHATSAPP_NUMBER + '?text=' + encoded, '_blank'); }
  } else {
...
function attachCheckoutButtons() {
  ...
  if (checkoutBtn && !checkoutBtn._wsAttached) { checkoutBtn.addEventListener('click', handler); checkoutBtn._wsAttached = true; }
  if (mobileCheckout && !mobileCheckout._wsAttached) { mobileCheckout.addEventListener('click', handler); mobileCheckout._wsAttached = true; }
  if (placeOrder && !placeOrder._wsAttached) { placeOrder.addEventListener('click', handler); placeOrder._wsAttached = true; }
  any.forEach(el => { if (!el._wsAttached) { el.addEventListener('click', handler); el._wsAttached = true; } });
}

/* ---------- Init ---------- */
function initSite() {
  loadCart(); renderProductsGrid(); updateUI(); attachDrawerHandlers(); attachCheckoutButtons();
}
